...
人工复核不再只隐藏在四个资格数字卡的点击行为里。AI 获客 Job 的详情操作区始终提供
“人工复核工作台”主入口；阶段 02 有待办时还会分别显示带数量的“打开人工复核队列”和
“查看待补资料候选”按钮。入口在筛选阶段运行中也可使用，打开后仍严格绑定当前 Job，并分别
传递 `manual_review` 或 `need_enrichment` 精确筛选；原四状态卡继续作为辅助筛选入口。
