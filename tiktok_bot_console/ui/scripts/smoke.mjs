...
const pipelineViewChecks = [
  ...
  ['all durable job states are recognizable', ['queued', 'running', 'waiting_decision', 'cancelling', 'succeeded', 'partial_failed', 'failed', 'interrupted', 'cancelled'].every(status => pipelineViewSrc.includes(`'${status}'`))],
  ...
  ['five-second Job polling remains authoritative', pipelineViewSrc.includes('window.setInterval(pollActiveJob, 5000)')],
  ['exactly one keyed Hermes monitor owns live transport', (pipelineViewSrc.match(/<HermesMissionMonitor\b/g) || []).length === 1 && pipelineViewSrc.includes(':key="selectedJob.id"') && !pipelineViewSrc.includes('subscribePipelineLiveEvents')],
  ['acquisition mode accepts current and compatible creator snapshots', pipelineViewSrc.includes("snapshot.businessMode === 'ai_acquisition'") && pipelineViewSrc.includes("snapshot.creatorSource === 'pipeline_ui'")],
  ['acquisition stages use business result components', pipelineViewSrc.includes('<StageDiscoveryResult') && pipelineViewSrc.includes('<StageQualificationResult') && pipelineViewSrc.includes('@filter-candidates=')],
  ['one same-Job candidate review drawer handles stage and monitor entry', (pipelineViewSrc.match(/<CandidateReviewDrawer\b/g) || []).length === 1 && pipelineViewSrc.includes('openReviewWorkbench') && pipelineViewSrc.includes('selectedJobId.value !== jobId')],
  ['raw stage JSON is isolated in one closed technical disclosure', (pipelineViewSrc.match(/<details\b[^>]*class="technical-diagnostics"/g) || []).length === 1 && !pipelineViewSrc.includes('class="stage-result"') && !/<details\b[^>]*class="technical-diagnostics"[^>]*\bopen\b/.test(pipelineViewSrc)],
  ['review mutations refresh only the selected Job stages', pipelineViewSrc.includes('stageRefreshToken.value += 1') && pipelineViewSrc.includes('handleReviewDataChanged') && pipelineViewSrc.includes('handleReviewComplete')],
