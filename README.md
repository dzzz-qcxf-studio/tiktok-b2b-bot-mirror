...
- API 文档: 以一键脚本输出的 API 端口为准，通常是 http://localhost:8000/docs

一键脚本会对后端候选端口 `8000, 8400–8409` 做真实 loopback bind 探测：8000 被 Windows
保留、占用或无法绑定时，自动选择首个可用备用端口。也可以用 `-BackendPort 8407` 明确指定。
前端固定使用 5173，并在启动时注入实际后端地址；脚本会通过只读、禁止缓存的
`/__tiktok-bot-runtime` 标记验证两者确实成对，不能把任意返回 200 的其他页面误认为本项目。
重复执行时优先复用已经配对且健康的服务，不会因端口状态后来变化而拉起第二套 API。
隐藏进程日志写入 Git 忽略的 `data/logs/`。
